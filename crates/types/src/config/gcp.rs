...
///
/// **Not live-reloadable.** Changing `broker-role-arn` or `session-name` on a running server (via
/// Restate's config file watcher) has no effect: the broker role assumed at the first install
